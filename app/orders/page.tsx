    ...
    return (
      <div className="px-4 py-8 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow border border-amber-100 space-y-2">
          <p className="text-sm text-gray-600">{lang === "hi"? "Apna mobile number daalo — purane orders aur tracking dikhenge" : "Enter your mobile number to see past orders and tracking"}</p>
  ...
  return (
    <div className="px-4 py-4 space-y-3">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-bold">📦 {lang === "hi"? "Mere Orders" : "My Orders"}</h2>
        <button onClick={logout} className="text-xs text-red-600 font-bold border border-red-200 rounded-full px-3 py-1">Logout</button>
      </div>
              ...
              <div className="flex items-center mt-2 mb-1">
                {STEPS.map((s, i) => (
                  <div key={s} className="flex-1 flex items-center">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text- font-bold ${i <= stepIdx? "bg-green-500 text-white" : "bg-gray-200 text-gray-400"}`}>{i <= stepIdx? "✓" : i + 1}</div>
                    {i < STEPS.length - 1 && <div className={`flex-1 h-1 mx-0.5 rounded ${i < stepIdx? "bg-green-500" : "bg-gray-200"}`} />}
                  </div>
                ))}
              </div>
            )}
